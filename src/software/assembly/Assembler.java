...
public class Assembler {
	...
	public Protogram assembleFile(File file) throws Exception {
		...
		// Statements for data should contain labels, statements for text may optionally have labels
		StatementCollection rawStatements = parseFile(file);
		
		// Error Checking
		boolean allProperlyFormed = validateStatements(rawStatements);
		if (!allProperlyFormed) throw new FileParseException();
		
		// Build program result
		Protogram result = new Protogram();
		
		// Translate .text statements into list of Instructions objects for easier linking
	...
	private boolean validateStatements(StatementCollection statements) throws Exception {
		...
		if (statements.text.size() == 0) throw new Exception("No instructions found in assembly file!");
